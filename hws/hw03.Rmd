...
head(planetsq2, 10)
ggplot(planetsq2, aes(x = year, y = min_mass, size = num_planets)) +
  geom_point() +
  scale_y_log10() +
  xlab("Year") +
  ylab("Minimum Mass") +
  ggtitle("Minimum Planet Mass Per Year")
```

**Describe the pattern between year and minimum mass of planet discovered using Radial Velocity.**

> There is no strong pattern between year and minimum planet mass discovered using radial velocity
...
planetsq3 = planets %>%
  ...
  count(method) %>%
  mutate(total = sum(n)) %>%
  summarize(method,n, proportion = n / total) %>%
  arrange(year)
```
...
```{r}
ggplot(planetsq3, aes(x = year, y = proportion, fill = method)) + 
  geom_col(color = "black") +
  xlab("Year") +
  ylab("Proportion by Method") +
  ggtitle("Spread of Discovery Method by Year")
```
...
> Radial velocity was originally most sucessful, but has been surpassed by transit in recent years
...
```{r}
planetsq3p2 = planetsq3 %>%
  filter(year >= 2010 & year <= 2022 & method != "Radial Velocity" & method != "Transit")
ggplot(planetsq3p2, aes(x = year, y = n, fill = method)) +
  geom_col(position = position_dodge2(preserve = "single")) +
  scale_x_continuous(breaks = seq(2010, 2022, 1), labels = seq(2010, 2022, 1)) +
  xlab("Year") +
  ylab("Number of Discoveries") +
  ggtitle("Number of Discoveries per Year by Method")
```
...
```{r}
officialq6b = official %>%
  mutate(month = month(date)) %>%
  select(month, prcp) %>%
  group_by(month) %>%
  drop_na(prcp) %>%
  mutate(n = n()) %>%
  summarize(month, n, pos = (prcp != 0)) %>%
  summarize(month, proportion = (sum(pos) / n)) %>%
  distinct()

official6bcheck = official %>%
  mutate(month = month(date)) %>%
  filter(month == 6)
```
...
```{r}
officialq6c = official %>%
  mutate(month = month(date), year = year(date)) %>%
  filter(year >= 1991 & year <= 2020) %>%
  group_by(month) %>%
  summarize(normal_mean = mean(tavg))
officialq6cpart2 = official %>%
  mutate(month = month(date), year = year(date)) %>%
  filter(year == 2021) %>%
  group_by(month) %>%
  drop_na() %>%
...
> My data doest have anything from 2022 for some reason but I ran it for 2021 and there were 8 months higher than average
...
```{r}
officialq6d = official %>%
  mutate(month = month(date), year = year(date)) %>%
  group_by(year) %>%
  drop_na() %>%
  summarize(year_mean = mean(tavg)) %>%
  arrange(desc(year_mean)) %>%
  slice_max(order_by = year_mean, n= 10)
```

>None of these years have occured since 2000
...
officialq7 = official %>%
  ...
  filter(month == 1) %>%
  group_by(year) %>%
  summarize(jan_avg = mean(tavg)) %>%
  arrange(desc(jan_avg)) %>%
  slice_max(order_by = jan_avg, n = 25)
print(officialq7, n = 26)
```

> 2023 January was very hot, as it would rank 8th hottest in all years since 1869
...
officialq8 = official %>%
  ...
  group_by(year) %>%
  summarize(jan_avg = mean(tavg))
ggplot(officialq8, aes(x = year, y = jan_avg)) +
  geom_smooth() + 
  geom_hline(yintercept = 27.02, color = "red", linetype = "dashed") +
  xlab("Year") +
  ylab("Average Temp") +
  ggtitle("Average Temperature in January by Year")
```
