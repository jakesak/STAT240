...
> The ^ is an exponential, raising 2 by each number is the sequence given.
...
> The median value is 17.2. This value is the median because the list is even, and therefore the median is average of the middle 2 values.
...
```{r}
data_mean = mean(x)
mean_diff = x - data_mean
squared_mean_diff = mean_diff^2
sum_squared_diffs = sum(squared_mean_diff)
sum_squared_diffs
```
